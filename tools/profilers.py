...
def depth(p):
    ...
    coordinates for the plot.
    """
    y = [z * -1 if z is not None else z for z in p["z"]]
    return p["l"], y

def setupHeight(dockwidget):
    dockwidget.invertY(True)
...
def setupSlope(dockwidget):
    dockwidget.invertY(False)
...
PLOT_PROFILERS = {u"Глубина": (depth, setupHeight),
                  u"Высота": (height, setupSlope),
                  u"Кривизна (%)": (slopes_pct, setupSlope),
                  u"Кривизна (°)": (slopes_deg, setupSlope)}
